...
const MUSIC_URL = '/music/yellow.mp3'; // Matches your directory
...
export default function OurStoryPage() {
  ...
  // 🛠️ MOBILE FIX: Added html5: true for better mobile browser support
  const [play, { stop }] = useSound(MUSIC_URL, { 
    volume: 0.4, 
    loop: true,
    html5: true 
  });
    ...
    return (
      <main className="min-h-screen w-full flex flex-col items-center justify-center p-6 bg-background relative overflow-hidden">
        ...
        >
          ...
          {/* 🛠️ MOBILE FIX: Wrapped in <form> to ensure mobile keyboard "Go/Enter" works and triggers audio */}
          <form onSubmit={handleLogin} className="w-full">
            <div className="relative w-full mb-6">
              <input
                type="text" // Changed to text to avoid some mobile browser password autofill bugs
                autoComplete="off"
                value={password}
  ...
  return (
    <main ref={mainRef} className="relative min-h-screen w-full font-sans antialiased text-white overflow-x-hidden">
      <FloatingElements />
      ...
      {/* 💛 HERO SECTION 💛 */}
      <section className="min-h-screen w-full flex flex-col items-center justify-center p-6 relative overflow-hidden text-center">
        <motion.div initial="hidden" animate="visible" variants={fadeInUp} className="z-10 flex flex-col items-center">
          <motion.div
      ...
      <section id="timeline" className="py-24 px-6 md:px-12 relative">
        <div className="hidden md:block absolute left-1/2 top-0 bottom-0 w-1 bg-white/10 -translate-x-1/2 z-0" />
        
        <motion.h2 
          className="text-4xl md:text-5xl font-extrabold text-center mb-20 text-white drop-shadow-sm"
      ...
      <section className="py-24 px-6 md:px-12 bg-black/30 backdrop-blur-sm relative border-y border-white/5">
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
          ...
          <motion.div variants={fadeInUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            <h2 className="text-4xl md:text-5xl font-extrabold mb-10 text-white">About Her <span className="text-romantic-accent">✨</span></h2>
          ...
          >
            ...
            <div className="relative rounded-3xl overflow-hidden border-4 border-white/10 aspect-[4/5] shadow-2xl">
              ...
              <p className="absolute bottom-6 left-6 text-xl md:text-2xl font-bold text-white drop-shadow-lg">Moments I never want to lose 💛</p>
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-24 px-6 md:px-12 relative min-h-[70vh] flex flex-col items-center justify-center">
        ...
        <motion.h2 
          className="text-4xl md:text-5xl font-extrabold text-center mb-16 text-white drop-shadow-yellow-glow z-10"
          variants={fadeInUp} initial="hidden" whileInView="visible" viewport={{ once: true }}
        >
          Future Dreams <Stars className="inline text-romantic-yellow"/>
        </motion.h2>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 max-w-6xl w-full z-10 text-center">
  ...
  return (
    ...
    >
      <div className={`w-full md:w-1/2 flex ${isRight ? 'md:justify-end' : 'md:justify-start'}`}>
        <div className={`relative rounded-3xl overflow-hidden border-2 border-white/10 w-full ${glow ? 'shadow-soft-glow border-romantic-accent/20' : ''}`}>
          <Image src={image} alt={title} width={400} height={300} className="object-cover w-full h-auto aspect-[4/3]" />
      ...
      <div className="hidden md:block absolute left-1/2 -translate-x-1/2 p-3 bg-background rounded-full border-2 border-white/10 z-20">
      ...
      <div className={`w-full md:w-1/2 flex flex-col ${isRight ? 'md:items-start text-left' : 'md:items-end text-right'}`}>
        <h4 className={`text-2xl md:text-3xl font-bold mb-4 ${glow ? 'text-transparent bg-clip-text bg-sunset-gradient' : 'text-white'}`}>{title}</h4>
...
function DreamCard({ title, description, emoji, delay }: any) {
  return (
    <motion.div
      ...
      whileHover={{ y: -10 }}
    >
      <span className="text-5xl md:text-6xl mb-6 block animate-float">{emoji}</span>
      <h5 className="text-xl md:text-2xl font-bold text-white mb-3">{title}</h5>
